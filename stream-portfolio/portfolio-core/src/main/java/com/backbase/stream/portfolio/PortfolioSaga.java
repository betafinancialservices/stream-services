...
public class PortfolioSaga implements StreamTaskExecutor<PortfolioTask> {
    ...
    private static final String UPSERT_AGGREGATE_PORTFOLIOS = "upsert-aggregate-portfolios";
    ...
    @ContinueSpan(log = UPSERT_AGGREGATE_PORTFOLIOS)
    private Mono<PortfolioTask> upsertAggregatePortfolios(@SpanTag(value = "streamTask") PortfolioTask task) {
        ...
        return ReactiveStreamHandler.getFluxStream(task.getData().getAggregatePortfolios())
            ...
            .onErrorResume(PortfolioBundleException.class,
                ReactiveStreamHandler.error(task, AGGREGATE_PORTFOLIO_ENTITY, UPSERT_AGGREGATE_PORTFOLIOS))
            .collectList()
            .map(o -> task);
    ...
    private Mono<PortfolioTask> upsertAssetClasses(@SpanTag(value = "streamTask") PortfolioTask task) {
        ...
        return ReactiveStreamHandler.getFluxStream(task.getData().getAssetClasses())
            ...
            .onErrorResume(PortfolioBundleException.class,
                ReactiveStreamHandler.error(task, ASSET_CLASS_ENTITY, UPSERT_ASSET_CLASSES))
            .collectList()
            .map(o -> task);
    }
    ...
    private Mono<PortfolioTask> upsertRegions(@SpanTag(value = "streamTask") PortfolioTask task) {
        ...
        return ReactiveStreamHandler.getFluxStream(task.getData().getRegions())
            ...
            .onErrorResume(PortfolioBundleException.class,
                ReactiveStreamHandler.error(task, REGION_ENTITY, UPSERT_REGIONS))
            .collectList()
            .map(o -> task);
    }
